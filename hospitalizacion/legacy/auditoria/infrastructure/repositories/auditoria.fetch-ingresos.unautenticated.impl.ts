...
import { AuditoriaOrm, InternacionOrm } from '@hpn/lgc/aud/orm/hpn/auditoria';
...
@Injectable()
export class AuditoriaFetchIngresosUnautenticatedImpl {
  async execute(contexto: GcmContextCode) {
    try {
      const conn = switchConn(gcmContextFactory(contexto));
      const now = TimerServices.removeTimeZone(new Date()).toISOString().split('T')[0];

      const dayInMs = 86400000;
      const days = dayInMs * 60;
      ...
      const internacionRp = conn.getRepository(InternacionOrm);
      ...
      const estanciasCerradas = await estanciaRp.find({
        where: [{ fechaEgreso: Between(nowOneDayLessWtFt, nowWtFt) }],
        order: { id: 'DESC' },
        relations: [
          'ingreso',
          'ingreso.paciente',
      ...
      estanciasCerradas.forEach(ec => {
        const existActual = estanciasActuales.filter(ea => {
          if (!ea.ingreso) {
            ea.ingreso = new IngresoOrm();
            ea.ingreso.consecutivo = '000000';
          }
          if (!ec.ingreso) {
            ec.ingreso = new IngresoOrm();
            ec.ingreso.consecutivo = '000000';
          }
          return ea.ingreso.consecutivo === ec.ingreso.consecutivo;
        });
      ...
      const auditoriasPrevias = await this._fetchAuditorias(pacientesIds, contexto);

      if (pacientesIds.length) {
        estanciasQuery = await conn.query(estanciasByConsecutivos(consecutivos));
      ...
      estancias.forEach(i => {
        if (i.ingreso.detalleContrato && i.ingreso.detalleContrato.codigo[0] === '8') {
      ...
      const response = estancias.map(i => {
        ...
        if (i.ingreso && i.ingreso.paciente) i.ingreso.paciente.setTypes();

        let estanciasByDoc: GcmGrouped<EstanciaByPaciente>[];

        if (i.ingreso.paciente) {
          estanciasByDoc = estanciasByDocumento.filter(e => e.key === i.ingreso.paciente.numeroDoc);
        }
        ...
        if (estancias) {
          estancias.rows.map(r => {
            if (!r.fechaEgreso) r.isEstanciaActual = true;
        ...
        const cam = i.cama ? i.cama : null;
        const pac = i.ingreso.paciente ? i.ingreso.paciente : null;
        const ne = 'NONEXIST';
        ...
        return {
          ...
          nombre: pac ? pac.nombreCompleto : ne,
          ...
          tipoReingreso,
          nombreRegimen: pac ? pac.regimen.getForHumans() : ne,
          estancias: estancias ? estancias.rows : [],
          paciente: {
            genero: pac ? generoTypeFactory(pac.generoCode) : ne,
            fechaNacimiento: pac ? pac.fechaNacimiento : ne,
            nombreCompleto: pac ? pac.nombreCompleto : ne,
            documento: {
              tipo: pac ? tipoDocumentoTypeFactory(pac.tipoDocumentoCode) : ne,
              lugarExpedicion: pac ? pac.lugarExpDoc : ne,
              numero: pac ? pac.numeroDoc : ne,
            },
            regimen: pac ? pac.regimen : ne,
          },
          codigoCama: cam ? cam.codigo : ne,
          cama: {
            id: cam ? cam.id : ne,
            codigo: cam ? cam.codigo : ne,
            nombre: cam ? cam.nombre : ne,
            grupo: {
              id: cam ? cam.grupo.id : ne,
              codigo: cam ? cam.grupo.codigo : ne,
              nombre: cam ? cam.grupo.nombre : ne,
            },
            subgrupo: {
              id: cam ? cam.subgrupo.id : ne,
              codigo: cam ? cam.subgrupo.codigo : ne,
              nombre: cam ? cam.subgrupo.nombre : ne,
            },
          },
          contrato: {
            tipo: i.ingreso.detalleContrato ? i.ingreso.detalleContrato.nombreTipoContrato : ne,
          },
          auditorias: audiReptsRows,
          primerAuditoria: audiPriRep.length ? audiPriRep[0] : null,
          ultimaAuditoria: audiPriRepRevert.length ? audiPriRepRevert[0] : null,
        };
      });
  ...
  private async _fetchAuditorias(pacientesIds: number[], contexto: GcmContextCode) {
    const conn = switchConn(gcmContextFactory(contexto));
    const auditoriaRp = conn.getRepository(AuditoriaOrm);
    ...
    auditoriasGrouped.forEach(a => {
      ...
      a.rows.forEach(r => {
        if (r.estanciasInactivas) {
          r.estanciasInactivas.forEach(ei => {
