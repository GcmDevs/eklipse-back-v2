...
export async function generateNotasMedicasPdf(payload: TrasladoPdfPayload): Promise<Buffer | null> {
  ...
  // 3. Registro de Notas Médicas
  Y += drawSectionHeader(doc, 'REGISTRO DE EVOLUCIONES MÉDICAS', M, Y, CW);

  // Tabla Header
  const colW = [22, 20, CW - 82, 40]; // Fecha, Hora, Nota, Responsable
  const labels = ['FECHA', 'HORA', 'EVOLUCIÓN / NOTA', 'MÉDICO'];
  ...
  // Filtrar notas médicas usando TIPOS_EMPLEADO fuertemente tipado
  const notasMed = todasLasNotas
    .filter(n => n?.usuario?.tipoEmpleadoCode === TIPOS_EMPLEADO.MEDICO.getCode())
    .sort((a, b) => new Date(a.fecha).getTime() - new Date(b.fecha).getTime());

  if (notasMed.length === 0) return null;

  doc.setFont(FM.h, F.n);
  doc.setFontSize(9);

  notasMed.forEach((nota, idx) => {
  ...
  // Espacio para Firma (Médico)
  chkPage(35);
  Y += 10;
  const sigW = 60;
  const sigX = M;

  const medicoAsignado = data.asignacionActual?.medico;
  const firmaMedico = getFirmaBufferFromCedula(medicoAsignado?.documento);
  ...
  doc.setFont(FM.h, F.b);
  doc.setFontSize(9);
  doc.text('FIRMA MÉDICO TRATANTE', sigX, Y + 19);
  doc.text('R.M.: ____________________', sigX, Y + 24);

  if (medicoAsignado) {
    doc.setFont(FM.h, F.n);
    doc.text(v(medicoAsignado.nombre).toUpperCase(), sigX, Y + 14);
  }
